...
def test_authorize_model_basic(session, oso, fixture_data):
    ...
    posts = session.query(Post).filter(authorize_model(oso, "user", "read", session, Post))

    assert posts.count() == 5
    assert posts.all()[0].contents == "foo public post"
    assert posts.all()[0].id == 0

    posts = session.query(Post).filter(authorize_model(oso, "user", "write", session, Post))

    assert posts.count() == 4
    assert posts.all()[0].contents == "foo private post"
    assert posts.all()[1].contents == "foo private post 2"

    posts = session.query(Post).filter(authorize_model(oso, "admin", "read", session, Post))
    assert posts.count() == 9

    posts = session.query(Post).filter(authorize_model(oso, "moderator", "read", session, Post))
    print_query(posts)
    assert posts.all()[0].contents == "private for moderation"
    assert posts.all()[1].contents == "public for moderation"

    posts = session.query(Post).filter(authorize_model(oso, "guest", "read", session, Post))
...
def tag_test_fixture(session):
    ...
    other_user_private_post = Post(contents="other user private", created_by=other_user, access_level="private")
...
def test_in_multiple_attribute_relationship(session, oso, tag_test_fixture):
    ...
    posts = session.query(Post).filter(authorize_model(oso, tag_test_fixture["user"], "read", session, Post))
...
def tag_nested_test_fixture(session):
    ...
    user_eng_post = Post(contents="user eng post", access_level="public", created_by=user, tags=[eng])
    user_user_post = Post(
    ...
    not_tagged_post = Post(contents="not tagged post", access_level="public", created_by=user, tags=[])
...
def test_nested_relationship_many_single(session, oso, tag_nested_test_fixture):
    ...
    posts = session.query(Post).filter(authorize_model(oso, tag_nested_test_fixture["user"], "read", session, Post))
    ...
    posts = session.query(Post).filter(
        authorize_model(oso, tag_nested_test_fixture["other_user"], "read", session, Post)
    )
...
def test_nested_relationship_many_many(session, oso, tag_nested_many_many_test_fixture):
    ...
    posts = session.query(Post).filter(
        authorize_model(oso, tag_nested_many_many_test_fixture["user"], "read", session, Post)
    )
    ...
    posts = session.query(Post).filter(
        authorize_model(oso, tag_nested_many_many_test_fixture["other_user"], "read", session, Post)
    )
...
def test_nested_relationship_many_many_many_constrained(session, engine, oso):
    ...
    posts = session.query(Post).filter(authorize_model(oso, bar, "read_2", session, Post))
    ...
    posts = session.query(Post).filter(authorize_model(oso, bar, "read_3", session, Post))
    print_query(posts)
    posts = posts.all()
...
def test_partial_in_collection(session, oso, tag_nested_many_many_test_fixture):
    ...
    posts = session.query(Post).filter(authorize_model(oso, user, "read", session, Post)).all()
    assert tag_nested_many_many_test_fixture["user_eng_post"] not in posts
    assert tag_nested_many_many_test_fixture["user_user_post"] not in posts
    assert tag_nested_many_many_test_fixture["random_post"] in posts
    assert tag_nested_many_many_test_fixture["not_tagged_post"] not in posts
...
def test_in_with_constraints_but_no_matching_objects(session, oso, tag_nested_many_many_test_fixture):
    oso.load_str(
        """
