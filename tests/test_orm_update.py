...
def test_update(engine, add_user, auth_handler):
    with Session(engine) as session:
        ...
        u.name = "Jane"
        session.commit()
    auth_handler.on_update.assert_called_once_with(u, {"name": "Jane"})
...
def test_update_multiple_same_column(engine, add_user, auth_handler):
    with Session(engine) as session:
        ...
        u.name = "Jill"
        session.commit()
    auth_handler.on_update.assert_any_call(u, {"name": "Jane"})
    auth_handler.on_update.assert_called_with(u, {"name": "Jill"})
    assert auth_handler.on_update.call_count == 2
...
def test_update_multiple_different_columns(engine, add_user, auth_handler):
    with Session(engine) as session:
        ...
        u.age = 43
        session.commit()
    auth_handler.on_update.assert_called_with(u, {"name": "Jane", "age": 43})
...
def test_update_rollback(engine, add_user, auth_handler):
    with Session(engine) as session:
        ...
        u.name = "Jane"
        session.rollback()
    auth_handler.on_update.assert_not_called()
